...
const AiAssistant: React.FC<AiAssistantProps> = ({ inventory, transactions, userRole }) => {
  ...
  const assistantInfo = useMemo(() => {
    ...
    // Gunakan hash dari role/name agar konsisten per sesi
  ...
  const handleSend = async (customPrompt?: string) => {
    ...
    try {
      ...
      setMessages(prev => {
        const newMessages = [...prev];
        newMessages[newMessages.length - 1].content = 'Maaf, terjadi gangguan pada server AI kami.';
        return [...newMessages];
      });
    } finally {
      setIsLoading(false);
      ...
      {isOpen && (
        <div className="absolute bottom-16 right-0 w-[380px] sm:w-[500px] h-[600px] bg-white rounded-3xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <div className="flex items-center space-x-1.5">
                  ...
                  <span className="text-[10px] text-slate-400 uppercase font-bold tracking-widest">{isLoading ? 'Menganalisis Data...' : 'Siap Membantu'}</span>
                </div>
              </div>
            </div>
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                <div className={`max-w-[90%] p-4 rounded-2xl text-sm shadow-sm transition-all ${
                  ...
                  {/* Sederhana: Rendering markdown dasar untuk tabel */}
                  <div className="prose prose-sm max-w-none prose-slate">
                    <p className="whitespace-pre-wrap leading-relaxed font-medium">
                      {m.content || (isLoading && i === messages.length - 1 ? 'Sedang memproses database...' : '')}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="p-4 bg-white border-t border-slate-100">
            <div className="flex space-x-2 bg-slate-50 p-1.5 rounded-2xl border border-slate-200 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:bg-white transition-all">
              <input 
                className="flex-1 px-3 py-2 outline-none text-sm bg-transparent text-slate-900 placeholder:text-slate-400"
                placeholder="Contoh: 'Tampilkan tabel stok laptop Apple'..."
                value={input}
              ...
              <button 
                ...
                className="p-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all disabled:opacity-50 shadow-md flex items-center justify-center active:scale-95"
              >
                {isLoading ? (
